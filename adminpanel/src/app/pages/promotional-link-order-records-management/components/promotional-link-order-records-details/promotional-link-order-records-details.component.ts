...
@Component({
  selector: 'promotional-link-order-records-details',
  templateUrl: './promotional-link-order-records-details.component.html',
})

  export class PromotionalLinkOrderRecordsDetailsComponent extends BaseComponent implements OnInit {
  ...
  clientOrderCount: any;
    ...
    constructor(private _sanitizer: DomSanitizer, private _service: PromotionalLinkService, private toastService: ToastrService, private router: ActivatedRoute) {
      super();
    ...
    ngOnInit(): void {
      ...
      this._service.getPromotionalLinkDataById(this.promotionalLinkId)
      .subscribe({
        next: (results: any) => {
          this.promotionalData = results;
      ...
      this._service.getClientOrderCountByPromotionalId(this.promotionalLinkId, startDate, endDate).subscribe({
        next: (res) => {
          this.clientOrderCount = res
        },
        error:(err) => {
          console.log(err)
        }
